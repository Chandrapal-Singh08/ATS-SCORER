...
from backend.core.model_loader import get_embedder   # ✅ NEW IMPORT
...
async def analyze_resume(
    ...
    embedder = get_embedder()   # ✅ Lazy loading
...
async def get_history(user_id: str = Depends(get_current_user)):
    """Return user's previous resume analyses."""
...
async def generate_history_pdf(
    ...
    try:
        ...
        logger.error(f"Failed to generate PDF for history: {exc}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to generate PDF: {exc}",
        )